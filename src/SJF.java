...
public class SJF extends JPanel{
	...
	public SJF(String[][] systemData,String[][] interactiveData,String[][] batchData,int systemCount,int intCount,int batchCount, int[] indexValues, int currentIndex) {
		...
		for(int i=0; i<count-1; i++) {
			for(int j=1; j<count-i-1; j++) {
				if(Integer.parseInt(newProcessData[j][2])>Integer.parseInt(newProcessData[j+1][2])) {
					if (Integer.parseInt(newProcessData[j][2])>Integer.parseInt(newProcessData[j][1])) {
		...
		newTimer = new Timer(50, new ActionListener() {
	...
	public ProgressBars() {
		...
		homeButton.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent arg0) {
				JFrame f1 = (JFrame) SwingUtilities.getWindowAncestor(SJF.this);
				f1.dispose();
				new App();
			}
		});
	...
	public void calculate(int atime[], int btime[]) {
	    ...
	    resultDisplay.setText("Algorithm : Shortest Job First(SJF)\nGantt Chart"+"\n");
	    for(int a=0; a<count; a++) {
